...
class BaseProcess:  # Be sure to make it a singleton (metaclass=Singleton)
    ...
    async def unload(self):
        ...
        if len(self.links):
            logging.warning(
                f"Unloading process that still has the {len(self.links)} links"
            )
        ...
        if self.process:
            ...
            logging.warning(
                f"Attempted to unload process {self.id} when it is already unloaded"
            )
    ...
    async def link(self, link_id):
        ...
        self.links.add(
            link_id
        )  # Add to links after loading process to ensure link established
