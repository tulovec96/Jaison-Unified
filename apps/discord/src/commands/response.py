...
class ResponseCommandGroup(BaseCommandGroup):
    def __init__(self, params={}):
        ...
        self.command_list = [request_response]


@discord.app_commands.command(
    name="request_response", description="Request a response."
)
async def request_response(interaction, output_audio: bool = False) -> None:
    try:
        ...
        reply = "Response job sent successfully: {}".format(
        ...
        await interaction.response.send_message(
            f"Failed to send response job: {str(err)}"
        )
