...
@discord.app_commands.command(
    name="operation_load", description="Load a specific operation"
)
async def operation_load(interaction, op_type: str, op_id: str) -> None:
    try:
        ...
        reply = "Operation load job sent successfully: {}".format(
        ...
        await interaction.response.send_message(
            f"Failed to send operation load job: {str(err)}"
        )


@discord.app_commands.command(
    name="operation_reload_from_config", description="Reload all operations"
)
async def operation_reload_from_config(interaction) -> None:
    try:
        ...
        parsed_response = response.json()
        reply = "Operation reload job sent successfully: {}".format(
            parsed_response["response"]["job_id"]
        )
        logging.info(reply)
        await interaction.response.send_message(reply)
        ...
        await interaction.response.send_message(
            f"Failed to send operation reload job: {str(err)}"
        )


@discord.app_commands.command(
    name="operation_unload", description="Unload a specific operation"
)
async def operation_unload(interaction, op_type: str, op_id: str = None) -> None:
    try:
        ...
        parsed_response = response.json()
        reply = "Operation unload job sent successfully: {}".format(
            parsed_response["response"]["job_id"]
        )
        logging.info(reply)
        await interaction.response.send_message(reply)
        ...
        await interaction.response.send_message(
            f"Failed to send operation unload job: {str(err)}"
        )
