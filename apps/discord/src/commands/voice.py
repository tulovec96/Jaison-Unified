...
class VoiceCommandGroup(BaseCommandGroup):
    def __init__(self, params={}):
        ...
        self.command_list = [join_vc, leave_vc]
...
async def join_vc(interaction, channel: discord.VoiceChannel) -> None:
    ...
    try:
        await _disconnect_client_if_connected(
            interaction.client
        )  # client cannot connect if it is already in a vc
        interaction.client.vc = await channel.connect(
        ...
        interaction.client.vc.listen(
            BufferSink(interaction.client.scheduler, interaction.client)
        )
...
@discord.app_commands.command(
    description="Leave current voice channel.", name="leave_vc"
)
async def leave_vc(interaction) -> None:
    ...
    try:
        ...
        await interaction.response.send_message(
            f"Failed to leave current voice channel..."
        )
        return
