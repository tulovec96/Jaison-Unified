...
class ManageCommandGroup(BaseCommandGroup):
    def __init__(self, params={}):
        ...
        self.command_list = [clear_history]
...
@discord.app_commands.command(
    name="clear_history", description="Clear cache conversation history."
)
async def clear_history(interaction) -> None:
    try:
        response = requests.delete(config.jaison_api_endpoint + "/api/context").json()
