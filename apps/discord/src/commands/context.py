...
async def context_request_add(interaction, content: str) -> None:
    try:
        ...
        reply = "Add request context job sent successfully: {}".format(
            parsed_response["response"]["job_id"]
        )
        logging.info(reply)
        ...
        await interaction.response.send_message(
            f"Failed to send add request context job: {str(err)}"
        )


@discord.app_commands.command(
    name="context_conversation_add", description="Send text dialogue"
)
async def context_conversation_add(interaction, user: str, content: str) -> None:
    try:
        ...
        parsed_response = response.json()
        reply = "Add conversation context job sent successfully: {}".format(
            parsed_response["response"]["job_id"]
        )
        logging.info(reply)
        await interaction.response.send_message(reply)
        ...
        await interaction.response.send_message(
            f"Failed to send add conversation context job: {str(err)}"
        )


@discord.app_commands.command(
    name="context_custom_register", description="Register a new custom context"
)
async def context_custom_register(
    interaction, context_id: str, name: str, description: str
) -> None:
    try:
        ...
        parsed_response = response.json()
        reply = "Register custom context job sent successfully: {}".format(
            parsed_response["response"]["job_id"]
        )
        logging.info(reply)
        await interaction.response.send_message(reply)
        ...
        await interaction.response.send_message(
            f"Failed to send register custom context job: {str(err)}"
        )


@discord.app_commands.command(
    name="context_custom_remove", description="Remove a custom context"
)
async def context_custom_remove(interaction, context_id: str) -> None:
    try:
        ...
        parsed_response = response.json()
        reply = "Remove custom context job sent successfully: {}".format(
            parsed_response["response"]["job_id"]
        )
        logging.info(reply)
        await interaction.response.send_message(reply)
        ...
        await interaction.response.send_message(
            f"Failed to send remove custom context job: {str(err)}"
        )


@discord.app_commands.command(
    name="context_custom_add", description="Add information to a custom context"
)
async def context_custom_add(interaction, context_id: str, content: str) -> None:
    try:
        ...
        parsed_response = response.json()
        reply = "Add custom context job sent successfully: {}".format(
            parsed_response["response"]["job_id"]
        )
        logging.info(reply)
        await interaction.response.send_message(reply)
        ...
        await interaction.response.send_message(
            f"Failed to send add custom context job: {str(err)}"
        )
