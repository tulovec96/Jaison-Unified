...
class InfoCommandGroup(BaseCommandGroup):
    def __init__(self, params={}):
        ...
        self.command_list = [get_loaded_operations, get_current_config]


@discord.app_commands.command(
    name="get_loaded_operations", description="List loaded operations"
)
async def get_loaded_operations(interaction) -> None:
    try:
        response = requests.get(config.jaison_api_endpoint + "/api/operations")
        ...
        await interaction.response.send_message(
            f"get_loaded_operations failed: {str(err)}"
        )


@discord.app_commands.command(
    name="get_current_config", description="List loaded configuration"
)
async def get_current_config(interaction) -> None:
    try:
        response = requests.get(config.jaison_api_endpoint + "/api/config")
        if response.status_code != 200:
            raise Exception("{} {}".format(response.status_code, response.reason))
        ...
        await interaction.response.send_message(
            f"get_current_config failed: {str(err)}"
        )
